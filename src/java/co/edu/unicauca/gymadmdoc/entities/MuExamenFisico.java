...
@NamedQueries({
   ...
   @NamedQuery(name = "MuExamenFisico.findByUsuIdentificacion", query = "SELECT m FROM MuExamenFisico m WHERE m.usuIdentificacion = :usuIdentificacion"),
...
public class MuExamenFisico implements Serializable {
   ...
   public MuExamenFisico(Long efisId, long usuIdentificacion, Date efisFecha, float efisPeso, float efisTalla, float efisFc, float efisFcm, float efisTa, float efisFr, float efisImc, float efisIcc, byte[] efisFotoCuerpoCompletoFrente, byte[] efisFotoCuerpoCompletoEspalda, byte[] efisFotoCuerpoCompletoPerfilDer, byte[] efisFotoCuerpoCompletoPerfilIzq) {
      this.efisId = efisId;
   ...
   public String toString() {
      return "co.edu.unicauca.gymadmdoc.entities.MuExamenFisico[ efisId=" + efisId + " ]";
   }
